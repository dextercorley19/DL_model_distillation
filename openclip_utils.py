...
def eval_embeddings_model(
    ...
    if torch.backends.mps.is_available():
        ...
        device = torch.device("cpu")
    print(f"Using device: {device} for eval_embeddings_model")
...
def train_student_classification_model(
    ...
    if torch.backends.mps.is_available():
        ...
        device = torch.device("cpu")
    print(f"Using device: {device} for train_student_classification_model")
...
def train_probe_model(
    ...
    # Determine device for probe_model
    if torch.backends.mps.is_available():
        device = torch.device("mps")
    elif torch.cuda.is_available():
        device = torch.device("cuda")
        ...
        device = torch.device("cpu")
    print(f"Using device: {device} for train_probe_model")
...
def train_model_from_scratch(
    ...
    # Determine the device
    if torch.backends.mps.is_available():  # For Apple Silicon (M1/M2 chips)
        device = torch.device("mps")
    elif torch.cuda.is_available():
        device = torch.device("cuda")
    else:
